...
const SYNTHETIC_CAPTURE: &str = concat!(
    ...
    "00000000000000000000000000000000000000000000000000000000000003f4",
    "00000000000000000000000000000000000000000000000000000000000003f5",
    "00000000000000000000000000000000000000000000000000000000000003f6",
    "00000000000000000000000000000000000000000000000000000000000003f7",
    "0000000000000000000000001000000000000000000000000000000000000001",
    "00000000000000000000000000000000000000000000000000000000000003f9",
    "00000000000000000000000000000000000000000000000000000000000003fa",
);
...
#[test]
fn synthetic_capture_pins_all_v3_lens_word_positions() {
    let view = decode_capture(SYNTHETIC_CAPTURE);
    let expected_id: B256 = "1111111111111111111111111111111111111111111111111111111111111111".parse().expect("id");
    assert_eq!(view.id, expected_id);
    assert!(!view.cancelled);
    assert!(view.filled);
    assert!(!view.nonceSpent);
    assert!(view.killedByOwner);
    assert!(!view.proxyDeployed);
    assert!(view.inWindow);
    assert!(!view.exclusiveWindow);
    assert!(view.decayComplete);
    assert_eq!(view.floorNow, U256::from(0x3f1u64));
    assert_eq!(view.feeNow, U256::from(0x3f2u64));
    assert_eq!(view.requiredNow, U256::from(0x3f3u64));
    assert_eq!(view.floorForOutsider, U256::from(0x3f4u64));
    assert_eq!(view.requiredForOutsider, U256::from(0x3f5u64));
    assert_eq!(view.ownerBalance, U256::from(0x3f6u64));
    assert_eq!(view.ownerProxyAllowance, U256::from(0x3f7u64));
    assert_eq!(view.proxy, parse_address("0x1000000000000000000000000000000000000001").expect("proxy"));
    assert_eq!(view.observedAt, U256::from(0x3f9u64));
    assert_eq!(view.observedBlock, U256::from(0x3fau64));
}
